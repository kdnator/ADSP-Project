...
for snri = 1:length(SNR)
    ...
    if(strcmp(noiseType, 'white') == 1)
        ...
        [y,fs] = audioread(strcat(soundpath, noiseType,'\', num2str(SNR(snri)),'dB\',filename,'_',noiseType,'_sn',num2str(SNR(snri)),'.wav'));
    end
    y=y';
    z=z';
    ...
    if strcmp(orderType,'fixed') == 1
        ...
        order = findOrder(segment,SNR(snri),noiseType,saveToPath);
    end
